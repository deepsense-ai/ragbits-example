...
if ! gcloud storage buckets describe "gs://$GCP_STATE_BUCKET" &>/dev/null; then
    echo "Creating bucket gs://$GCP_STATE_BUCKET..."
    gcloud storage buckets create "gs://$GCP_STATE_BUCKET" --location="$GCP_REGION" --project="$GCP_PROJECT_ID"
    gcloud storage buckets update "gs://$GCP_STATE_BUCKET" --versioning
else
    echo "Bucket gs://$GCP_STATE_BUCKET already exists."
fi

echo "Initializing Infrastructure..."
cd infrastructure/gcp/terraform
tofu init -reconfigure -backend-config="bucket=$GCP_STATE_BUCKET"
